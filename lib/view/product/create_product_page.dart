// lib/view/product/create_product_page.dart
...
class CreateProductPage extends StatefulWidget {
  const CreateProductPage({
    ...
    this.isEdit = false,
    this.editId,
    // initial values (dipakai saat edit)
    this.initialName,
    ...
    this.initialMinStockLevel,
    this.initialImageUrl, // hanya untuk preview; tidak dikirim jika user tidak ganti
  });
  ...
  final int? initialMinStockLevel;
...
class _CreateProductPageState extends State<CreateProductPage> {
  ...
  // image
  File? _imageFile;                 // gambar baru yang dipilih
  String? _initialImageUrl;         // preview gambar lama (edit)
  ...
  void _prefillIfEdit() {
    ...
    _brand.text     = widget.initialBrand ?? '';
    _size.text      = widget.initialSize ?? '';
    _color.text     = widget.initialColor ?? '';
    _stock.text     = (widget.initialStockQuantity ?? 0).toString();
    _minStock.text  = (widget.initialMinStockLevel ?? 0).toString();
  ...
  Future<void> _pickImage() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        _imageFile = file;       // user mengganti gambar
        _initialImageUrl = null; // sembunyikan preview lama
      });
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<String?> _validateImage(File file) async {
    ...
    if (bytes > max) {
      return 'Ukuran gambar maksimal 2MB. File ${(bytes/1024/1024).toStringAsFixed(2)}MB';
    }
    final mime = lookupMimeType(file.path) ?? '';
    const allowed = ['image/png','image/jpg','image/jpeg','image/gif'];
    if (!allowed.contains(mime)) return 'Format tidak didukung. Gunakan PNG/JPG/JPEG/GIF';
    return null;
  }
  ...
  TextStyle get _h1      => const TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: Color(0xFF111827));
  TextStyle get _subtitle=> const TextStyle(fontSize: 13, color: Color(0xFF6B7280), fontWeight: FontWeight.w600);
  TextStyle get _section => const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Color(0xFF111827));
  TextStyle get _label   => const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF6B7280));
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (widget.isEdit) {
        ...
        if (id == null) {
          throw 'ID produk tidak ditemukan';
        }
        resp = await vm.updateProduct(
          id: id,
          ...
          imageFile: _imageFile, // hanya terkirim kalau user ganti
        );
      } else {
        // CREATE (kode milikmu yang lama)
        resp = await vm.createProduct(
          name: _name.text.trim(),
          categoryId: _selectedCategoryId!,
          description: _desc.text.trim().isEmpty ? null : _desc.text.trim(),
          brand: _brand.text.trim().isEmpty ? null : _brand.text.trim(),
      ...
      if (resp.code == 200 || resp.code == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(resp.message ?? 'Operation failed'), behavior: SnackBarBehavior.floating),
        );
      }
    } catch (e) {
      if (!mounted) return;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(isEdit ? 'Edit Product' : 'Create Product', style: _h1),
              const SizedBox(height: 4),
              Text(isEdit ? 'Update this product information' : 'Add a new product to your inventory', style: _subtitle),
                    ...
                    children: [
                      ...
                      if (_loadingCats) ...[
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: Text('Gagal memuat kategori: $_loadCatsError',
                                  style: const TextStyle(color: Colors.red, fontSize: 12, fontWeight: FontWeight.w700)),
                            ),
                            TextButton(onPressed: () { setState(() { _loadingCats = true; _loadCatsError = null; }); getCategory(); }, child: const Text('Coba lagi')),
                          ],
                        ),
                        const SizedBox(height: 6),
                      ...
                      const SizedBox(height: 6),
                      TextFormField(controller: _brand, textInputAction: TextInputAction.next, decoration: _dec('Enter brand')),
                      const SizedBox(height: 12),

                      Text('Size', style: _label),
                      const SizedBox(height: 6),
                      TextFormField(controller: _size, textInputAction: TextInputAction.next, decoration: _dec('Enter size')),
                      const SizedBox(height: 12),

                      Text('Color', style: _label),
                      const SizedBox(height: 6),
                      TextFormField(controller: _color, textInputAction: TextInputAction.next, decoration: _dec('Enter color')),
                      const SizedBox(height: 12),
                      ...
                      LayoutBuilder(
                        builder: (context, c) {
                          final left = Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Stock Quantity *', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF6B7280))),
                              const SizedBox(height: 6),
                              TextFormField(
                                controller: _stock,
                          ...
                          final right = Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Min Stock Level *', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF6B7280))),
                              const SizedBox(height: 6),
                              TextFormField(
                      ...
                      if (_imageFile == null && (_initialImageUrl ?? '').isNotEmpty)
                        Stack(
                          children: [
                            ClipRRect(
                              borderRadius: BorderRadius.circular(12),
                              child: Image.network(_initialImageUrl!, height: 180, width: double.infinity, fit: BoxFit.cover),
                            ),
                            Positioned(
                              right: 8, top: 8,
